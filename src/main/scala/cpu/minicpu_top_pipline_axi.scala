...
class IF_stage extends Module {
  val io = IO(new Bundle {
    val inst_req = Output(Bool())//请求//not_finish
    val inst_addr_ok= Input(Bool())//请求地址(写数据传输)传输ok//not_finish
    val inst_data_ok= Input(Bool())//请求读数据(写数据)ok//not_finish
...
class AXI_CPU extends Module {
  val io = IO(new Bundle {
    ...
    val cpu_inst_size   = Output(UInt(2.W))//字节数:0->1,1->2,2->4
    val cpu_inst_wstrb  = Output(UInt(4.W))//写使能
    val cpu_inst_addr   = Output(UInt(32.W))//请求地址
    val cpu_inst_wdata  = Output(UInt(32.W))//写数据
    val cpu_inst_addr_ok= Input(Bool())//请求地址(写数据传输)传输ok
    val cpu_inst_data_ok= Input(Bool())//请求读数据(写数据)ok
    val cpu_inst_rdata  = Input(UInt(32.W))//请求读取的数据

    val cpu_data_req    = Output(Bool())//请求
    val cpu_data_wr     = Output(Bool())//1写请求,0读请求
    val cpu_data_size   = Output(UInt(2.W))//字节数:0->1,1->2,2->4
    val cpu_data_wstrb  = Output(UInt(4.W))//写使能
    val cpu_data_addr   = Output(UInt(32.W))//请求地址
    val cpu_data_wdata  = Output(UInt(32.W))//写数据
    val cpu_data_addr_ok= Input(Bool())//请求地址(写数据传输)传输ok
    val cpu_data_data_ok= Input(Bool())//请求读数据(写数据)ok
    val cpu_data_rdata  = Input(UInt(32.W))//请求读取的数据
  ...
  //cpu取指令
  io.cpu_inst_req  := if_stage.io.inst_req
  io.cpu_inst_wr   := false.B
  io.cpu_inst_size := 2.U(2.W)
  io.cpu_inst_wstrb:= 0.U(4.W)
  io.cpu_inst_addr := if_stage.io.inst_sram_addr
  io.cpu_inst_wdata:= 0.U(32.W)
  if_stage.io.inst_addr_ok :=io.cpu_inst_addr_ok
  if_stage.io.inst_data_ok :=io.cpu_inst_data_ok
  if_stage.io.inst_sram_rdata:=io.cpu_inst_rdata
...
//   if_stage.io.inst_sram_rdata := io.inst_sram_rdata
  ...
  io.cpu_data_addr := exe_stage.io.mem_addr
  io.cpu_data_wdata:= st_data.asUInt
  exe_stage.io.cpu_data_addr_ok:=io.cpu_data_addr_ok
  exe_stage.io.cpu_data_data_ok:=io.cpu_data_data_ok
  mem_stage.io.mem_value := io.cpu_data_rdata.asSInt
